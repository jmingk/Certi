...
for t in range(1,T+1):
    ...
    A -= 1
    B -= 1
    reception_list = list(map(int, input().split())) # 각 접수창구 처리시간
    repaircenter_list = list(map(int, input().split())) # 각 정비창구 처리시간
    arrival_time_list = list(map(int, input().split())) # 각 고객의 도착시간
    ...
    users = [] # 접수창구를 이용한 고객들
    for i in range(k):
        time = arrival_time_list[i] # 현재 고객의 도착시간
        ...
        if temp: # 이용 가능한 접수창구 존재 -> 번호 낮은 애들 순으로 저장 중.
            ...
            end_time = time + reception_list[idx] # 현재 고객이 접수창구가 끝나는 시간
            ...
            end_time += reception_list[idx] # 가장 먼저 끝나는 접수창구 시간 + 접수창구 시간
    ...
    while users: # 먼저 도착한 순서대로 꺼내서 진행 -> 도착 시간 같은 경우 접수창구 번호 우선
        ...
        if temp: # 이용 가능한 정비창구 존재
            ...
            end_timeB = end_timeA + repaircenter_list[idxB]  # 정비창구 끝나는 시간
            ...
            end_timeB += repaircenter_list[idxB]
            heapq.heappush(heapB, (end_timeB, idxB))
            res.append((userIdx, idxA,idxB))
